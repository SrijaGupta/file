...
# -----------------------------------------------------------------------
    ...
    def load_configuration_from_template(self,
        ...
        if 'force_synchronize' in list(kvargs.keys()):
            force_synchronize = kvargs['force_synchronize']
        if 'synchronize' in list(kvargs.keys()):
            synchronize = kvargs['synchronize']
        if 'full' in list(kvargs.keys()):
            full = kvargs['full']
        ...
        if 'parallel' in list(kvargs.keys()):
    ...
    def jsnap(self, **kvargs):
        ...
        if 'section' in list(kvargs.keys()):
    ...
    def commands_executor(self, **kvargs):
        ...
        regex = ''
        xpath = ''
        if 'xpath' in list(kvargs.keys()):
            xpath = kvargs['xpath']

        if 'regex' in list(kvargs.keys()):
            regex = kvargs['regex']
