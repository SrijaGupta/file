...
class TestOpenssl(unittest.TestCase):
    ...
    @patch('jnpr.toby.utils.linux.openssl.update_index_file')
    ...
    def test_gen_cert_req_strongswan(self):
        self.linux_handle.log = MagicMock(return_value=True)
        lst = [Response("Generated"), Response("success"), Response("success")]
        self.linux_handle.shell = MagicMock(side_effect=lst)
        self.linux_handle.download = MagicMock(return_value=True)
        self.assertTrue(Openssl._gen_cert_req_strongswan(self.openssl_obj, self.linux_handle, cert_id='test_local',gen_on='strongswan'))
        try:
            Openssl._gen_cert_req_strongswan(self.openssl_obj, self.linux_handle, gen_on='strongswan')
        except Exception as err:
            self.assertEqual(err.args[0], "cert_id parameter is mandatory")
        lst = [Response("Generated"), Response("Generated"), Response("error")]
        self.linux_handle.shell = MagicMock(side_effect=lst)
        try:
            Openssl._gen_cert_req_strongswan(self.openssl_obj, self.linux_handle, cert_id='test_local',gen_on='strongswan')
        except Exception as err:
            self.assertEqual(err.args[0], "Generating cert request failed error")

    #@mock.patch("open", create=True)
    #@mock.patch("write", create=True)
    #@patch("close", create=True)
    @patch('jnpr.toby.utils.linux.openssl.update_index_file')
    @patch('jnpr.toby.utils.linux.openssl.gen_crl')
    ...
    def test_load_cert(self):
        lst = [Response("success"), Response("success"), Response("success"), Response("success"), Response("success")]
        self.device_obj.log = MagicMock(return_value=True)
        self.device_obj.cli = MagicMock(side_effect=lst)
    ...
    def test_gen_crl(self):
        ...
        try:
            lst = [Response('success'), Response('success'),Response('error'), Response('True'), Response('True'), Response('True')]
            self.linux_handle.shell = MagicMock(side_effect=lst)
    ...
    @patch('jnpr.toby.utils.linux.openssl.start_ocsp_responder')
