...
class TestAvalanche(unittest.TestCase):
    ...
    @patch('jnpr.toby.hldcl.trafficgen.avalanche.avalanche.Avalanche._get_version')
    def test_avalanche_init(self, get_version_mock, host_log_patch, super_mock):
        super_mock.return_value = True
        get_version_mock.return_value = '4.77'
        sys.modules['av'] = MagicMock()
        avalanche_obj = Avalanche(system_data=create_system_data(), avalanche_lib_path='/foo', avalanche_tcl_bin='foo', avalanche_threats='/foo', avalanche_jre_path='/foo')
        avalanche_obj.add_intf_to_port_map(intf_to_port_map={'intf1':'1/1'})
        try:
            avalanche_obj.connect(port_list=['1/1'])
            avalanche_obj.invoke(command='test')
        except Exception:
...
def create_system_data():
    ...
    system_data['system']['primary']['controllers']['re0']['user'] = 'admin'
    system_data['system']['primary']['controllers']['re0']['password'] = 'spt_admin'
...
if __name__ == '__main__':
    SUITE = unittest.TestLoader().loadTestsFromTestCase(TestAvalanche)
