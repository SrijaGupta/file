...
try:
		...
		term1match1 = AclEntryMatchInet6(match_dst_ports=[match2],match_src_ports=[match1])
                ...
                nt=AclEntryInet6NonTerminatingAction(action_count = AclActionCounter(counter_name="C1"),action_syslog=1,action_log=1,action_sample=1)
                ...
                filter=AccessList(acl_name = filtname,acl_type = ACL_TYPE_CLASSIC, acl_family = ACL_FAMILY_INET6, acl_flag = ACL_FLAGS_NONE, ace_list=[tlist1])
                print filter
                ...
                pause()
                filter = AccessList(acl_name=filtname,acl_family = ACL_FAMILY_INET6)
                print filter
