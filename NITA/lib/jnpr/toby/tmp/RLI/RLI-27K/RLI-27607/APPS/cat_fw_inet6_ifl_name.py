...
log_file_name = 'cat_' + 'Fw_inet6_ifl_name' + '_' + timestr + '.log'
...
client_id = 'Fw_inet6_ifl_name'  # App Id (Will be replaced by test name)
...
try:
    ...
    pause() 
    api_request = AccessListObjBind(bind_direction=1, bind_family=2, obj_type=1, bind_object=AccessListBindObjPoint(intf=args.iflname + '.0'), acl=AccessList(ace_list=[AclEntry(inet6_entry=AclInet6Entry(matches=AclEntryMatchInet6(ifl_names=[AclMatchIflNameIndex(ifl_name=args.iflname + '.0')]), ace_name="t1", adjacency=AclAdjacency(type=1), actions=AclEntryInet6Action(actions_nt=AclEntryInet6NonTerminatingAction(action_count=AclActionCounter(counter_name="C1")), action_t=AclEntryInet6TerminatingAction(action_accept=1)), ace_op=1))], acl_family=2, acl_flag=0, acl_name="f1", acl_type=1))
    print api_request 
