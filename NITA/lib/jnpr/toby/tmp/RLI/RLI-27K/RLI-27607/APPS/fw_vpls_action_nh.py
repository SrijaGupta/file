...
try:
    ...
    res = []
    IP2 = IpAddress(addr_string='10::1')

    matchIP2 = AclMatchIpAddress(addr=IP2, prefix_len=128, match_op=ACL_MATCH_OP_EQUAL)

    term1match1 = AclEntryMatchVpls(match_dst_v6_addrs=[matchIP2])

    nh = AclActionSetNexthop(nh_idx=655311226)
    t = AclEntryVplsTerminatingAction(action_nh=nh)
    #nt = AclEntryVplsNonTerminatingAction(action_nh = nh)
    term1Action1 = AclEntryVplsAction(action_t=t)

    adj = AclAdjacency(type=ACL_ADJACENCY_BEFORE)
    term1 = AclVplsEntry(ace_name="t1", ace_op=ACL_ENTRY_OPERATION_ADD, adjacency=adj, matches=term1match1,
                         actions=term1Action1)
    tlist1 = AclEntry(vpls_entry=term1)

    filter = AccessList(acl_name='f1', acl_type=ACL_TYPE_CLASSIC, acl_family=ACL_FAMILY_VPLS,
