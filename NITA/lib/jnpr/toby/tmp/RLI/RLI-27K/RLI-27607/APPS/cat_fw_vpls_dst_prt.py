...
log_file_name = 'cat_' + 'Fw_vpls_dst_prt' + '_' + timestr + '.log'
...
client_id = 'Fw_vpls_dst_prt'  # App Id (Will be replaced by test name)
...
try:
    ...
    pause() 
    api_request = AccessListObjBind(bind_direction=1, bind_family=4, obj_type=1, bind_object=AccessListBindObjPoint(intf=args.iflname + '.0'), acl=AccessList(ace_list=[AclEntry(vpls_entry=AclVplsEntry(matches=AclEntryMatchVpls(match_dst_ports=[AclMatchPort(max=0, match_op=1, min=0)]), ace_name="t1", adjacency=AclAdjacency(type=1), actions=AclEntryVplsAction(actions_nt=AclEntryVplsNonTerminatingAction(action_count=AclActionCounter(counter_name="C1")), action_t=AclEntryVplsTerminatingAction(action_accept=1)), ace_op=1))], acl_family=4, acl_flag=0, acl_name="f1", acl_type=1))
    print api_request 
