...
log_file_name = 'cat_' + 'Fw_vpls_ifl_name' + '_' + timestr + '.log'
...
client_id = 'Fw_vpls_ifl_name'  # App Id (Will be replaced by test name)
...
try:
    ...
    pause() 
    api_request = AccessListObjBind(bind_direction=1, bind_family=4, obj_type=1, bind_object=AccessListBindObjPoint(intf=args.iflname + '.0'), acl=AccessList(ace_list=[AclEntry(vpls_entry=AclVplsEntry(matches=AclEntryMatchVpls(ifl_names=[AclMatchIflNameIndex(ifl_name=args.iflname + '.0')]), ace_name="t1", adjacency=AclAdjacency(type=1), actions=AclEntryVplsAction(actions_nt=AclEntryVplsNonTerminatingAction(action_count=AclActionCounter(counter_name="C1")), action_t=AclEntryVplsTerminatingAction(action_accept=1)), ace_op=1))], acl_family=4, acl_flag=0, acl_name="f1", acl_type=1))
    print api_request 
