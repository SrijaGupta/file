...
try:
	...
	pause()
	print "################################################################################################ Classifier ADD COS1 Inet ################"
	print "################################################################################################################################"
	...
	pause()
	print "################################################################################################ Classifier Change ATR DELETE ################"
	print "################################################################################################################################"
	...
	api_request = CosClassifier(classifier_name	="cos", 
				    classifier_type	=classifier_rewrite_type_INET,)
				    #ref_objects=[CosObjRefInfo(referencing_obj_count=1, referencing_obj_type="classifier_rewrite_type_INET", referencing_obj_name="cos1")], status=EOK)
	#print api_request
	...
	api_request = CosClassifier(classifier_name="cos", classifier_type=classifier_rewrite_type_INET)
	...
	print "################################################################################################################################"
    	...
    	api_request = CosClassifier(classifier_name="cos", classifier_type=FEATURE_CP_TYPE_INET_PRECEDENCE,)# rule=[ClassifierRule(code_points=[1], forwarding_class_name="be1")])
	...
	print "################################################################################################################################"
    	...
    	api_request = CosClassifier(classifier_name="cos1", classifier_type=FEATURE_CP_TYPE_INET_PRECEDENCE,)# rule=[ClassifierRule(code_points=[1], forwarding_class_name="be1")])
    	result = cos.ClassifierDelete(api_request, api_timeout)
	print 'Invoking cos.ClassifierDelete \nreturn = ', result 
