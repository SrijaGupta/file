...
try:
 	...
 	api_request = CosRewrite(rewrite_name	="cos1", 
				    ...
				    rule		=[RewriteRule(operation = operation_ADD ,
									forwarding_class_name="be1", ## STRING
					   				...
					   				code_point_options=FeatureCodePoint(code_point=2))])
				    ####ref_objects=[CosObjRefInfo(referencing_obj_count=1, referencing_obj_type="ss", referencing_obj_name="co")], status=EOK)
        print api_request
	result = cos.RewriteAdd(api_request, api_timeout)
	print 'Invoking fw.RewriteAdd \nreturn = ', result
	...
	rewrite_family_EXP_MPLS_INET_BOTH	=  NODE_FEATURE_RW_EXP_MPLS_INET_BOTH
	rewrite_family_MPLS_INET_BOTH_NON_VPN	=  NODE_FEATURE_RW_EXP_MPLS_INET_BOTH_NON_VPN
	...
	print "################################################################################################ rewrite NODE with only node_family_features################"
	print "################################################################################################################################"
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 ...
					 node_family_features=[CosNodeFamilyFeature(feature_option=CosNodeFeatureOption( rewrite_family = rewrite_family_EXP ),
										    operation = operation_ADD,
										    direction=direction_EGRESS,    
										    feature_object_name = "cos1",
					 					    feature_type=feature_type_rewrite_rule)],)
					#node_features=[CosNodeFeature(direction=direction_EGRESS,
					#				 operation=operation_ADD,
					#				 feature_type=feature_type_TCP,
					#			         feature_val=CosNodeFeatureVal(object_name="cos1"))])
	...
	pause()
	print "################################################################################################ rewrite NODE  cos1 with only node_family_features################"
	print "################################################################################################################################"
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 node_name=args.iflname + ".0",
					 node_parent_type=CosNodeType_INTERFACE,
	...
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 ...
					 node_family_features=[CosNodeFamilyFeature(feature_option=CosNodeFeatureOption( rewrite_family = rewrite_family_MPLS_INET_BOTH_NON_VPN),
										    operation = operation_ADD,
										    direction=direction_EGRESS,
										    feature_object_name = "cos1",
					 					    feature_type=feature_type_rewrite_rule)],)
...
#					 					    feature_type=feature_type_rewrite_rule)],)				
	...
	print "################################################################################################################################"
    	...
    	api_request = CosRewrite(rewrite_name="cos", rewrite_type=classifier_rewrite_type_EXP,)# rule=[RewriteRule(code_points=[1], forwarding_class_name="be1")])
	...
	print "################################################################################################################################"
    	...
    	api_request = CosRewrite(rewrite_name="cos1", rewrite_type=classifier_rewrite_type_EXP,)# rule=[RewriteRule(code_points=[1], forwarding_class_name="be1")])
    	result = cos.RewriteDelete(api_request, api_timeout)
	print 'Invoking cos.RewriteDelete \nreturn = ', result 
