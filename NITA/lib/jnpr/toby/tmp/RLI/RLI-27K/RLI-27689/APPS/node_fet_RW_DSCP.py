...
try:
	...
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 ...
					 node_family_features=[CosNodeFamilyFeature(feature_option=CosNodeFeatureOption( rewrite_family = rewrite_family_DSCP ),
	...
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 ...
					 node_family_features=[CosNodeFamilyFeature(feature_option=CosNodeFeatureOption( rewrite_family = rewrite_family_DSCP_MPLS ),
										    operation = operation_ADD,
										    direction=direction_EGRESS,
										    feature_object_name = "cos1",
					 					    feature_type=feature_type_rewrite_rule)],)
...
#					 					    feature_type=feature_type_rewrite_rule)],)				
	...
	print "################################################################################################################################"
    	...
    	api_request = CosRewrite(rewrite_name="cos", rewrite_type=classifier_rewrite_type_DSCP,)# rule=[RewriteRule(code_points=[1], forwarding_class_name="be1")])
	...
	print "################################################################################################################################"
    	...
    	api_request = CosRewrite(rewrite_name="cos1", rewrite_type=classifier_rewrite_type_DSCP,)# rule=[RewriteRule(code_points=[1], forwarding_class_name="be1")])
    	result = cos.RewriteDelete(api_request, api_timeout)
	print 'Invoking cos.RewriteDelete \nreturn = ', result 
