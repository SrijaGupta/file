...
try:
	...
	print "################################################################################################ Purge time Get ################"
	print "################################################################################################################################"
	


	api_request = CosPurgeTime()
	print api_request
    	result = cos.CosPurgeTimeGet(api_request, api_timeout)
    	print 'Invoking cos.CosPurgeTimeGet \nreturn = ', result
	
	if result.status is EOK :
            print "CosPurgeTimeGet RPC Passed"
    	else:
            print "CosPurgeTimeGet RPC Failed"
	
	pause()


	print "################################################################################################ Purge time Change ################"
	print "################################################################################################################################"
	


	api_request = CosPurgeTime(purge_time = 30)
	print api_request
    	result = cos.CosPurgeTimeChange(api_request, api_timeout)
    	print 'Invoking cos.CosPurgeTimeChange \nreturn = ', result
	
	if result.status is EOK :
            print "CosPurgeTimeChange RPC Passed"
    	else:
            print "CosPurgeTimeChange RPC Failed"
