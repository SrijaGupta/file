...
try:
	...
	print "################################################################################################################################"
 	api_request = CosClassifier(classifier_name	="cos", 
				    classifier_type	=classifier_rewrite_type_DSCP,
				    sharable		=True, ######### FALSE
	...
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 ...
					 node_family_features=[CosNodeFamilyFeature(feature_option=CosNodeFeatureOption( classifier_family = classifier_family_DSCP ),
	...
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 ...
					 node_family_features=[CosNodeFamilyFeature(feature_option=CosNodeFeatureOption( classifier_family = classifier_family_DSCP_MPLS ),
										    operation = operation_ADD,
										    direction=direction_INGRESS,
										    feature_object_name = "cos1",
					 					    feature_type=feature_type_classifier_rule)],)
...
#					 					    feature_type=feature_type_classifier_rule)],)				
	...
	print "################################################################################################################################"
    	...
    	api_request = CosClassifier(classifier_name="cos", classifier_type=classifier_rewrite_type_DSCP,)# rule=[ClassifierRule(code_points=[1], forwarding_class_name="be1")])
	...
	print "################################################################################################################################"
    	...
    	api_request = CosClassifier(classifier_name="cos1", classifier_type=classifier_rewrite_type_DSCP,)# rule=[ClassifierRule(code_points=[1], forwarding_class_name="be1")])
    	result = cos.ClassifierDelete(api_request, api_timeout)
	print 'Invoking cos.ClassifierDelete \nreturn = ', result 
