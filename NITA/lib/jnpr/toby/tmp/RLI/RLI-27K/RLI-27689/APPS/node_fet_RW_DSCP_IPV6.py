...
try:
	...
	print "################################################################################################ Rewrite ADD COS1 DSCP_IPV6 ################"
	print "################################################################################################################################"
	
 	api_request = CosRewrite(rewrite_name	="cos", 
				    rewrite_type	=classifier_rewrite_type_DSCP_IPV6,
				    sharable		=True, ######### FALSE
	...
	pause()
	print "################################################################################################ Rewrite ADD COS DSCP_IPV6 ################"
	print "################################################################################################################################"
 	api_request = CosRewrite(rewrite_name	="cos1", 
				    ...
				    rule		=[RewriteRule(operation = operation_ADD ,
									forwarding_class_name="be1", ## STRING
					   				...
					   				code_point_options=FeatureCodePoint(code_point_str="010101"))])
				    ####ref_objects=[CosObjRefInfo(referencing_obj_count=1, referencing_obj_type="ss", referencing_obj_name="co")], status=EOK)
        print api_request
	result = cos.RewriteAdd(api_request, api_timeout)
	print 'Invoking fw.RewriteAdd \nreturn = ', result
	...
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 ...
					 node_family_features=[CosNodeFamilyFeature(feature_option=CosNodeFeatureOption( rewrite_family = rewrite_family_DSCP_IPV6 ),
	...
	pause()
	print "################################################################################################ rewrite NODE change with only node_family_features################"
	print "################################################################################################################################"
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 node_name=args.iflname + ".0",
					 node_parent_type=CosNodeType_INTERFACE,
	...
	api_request = CosNodeBindFeatures(node_type=CosNodeType_LOGICAL_INTERFACE ,
					 ...
					 node_family_features=[CosNodeFamilyFeature(feature_option=CosNodeFeatureOption( rewrite_family = rewrite_family_DSCP_IPV6_MPLS ),
										    operation = operation_ADD,
										    direction=direction_EGRESS,
										    feature_object_name = "cos1",
					 					    feature_type=feature_type_rewrite_rule)],)
...
#					 					    feature_type=feature_type_rewrite_rule)],)				
	...
	print "################################################################################################################################"
    	...
    	api_request = CosRewrite(rewrite_name="cos", rewrite_type=classifier_rewrite_type_DSCP_IPV6,)# rule=[RewriteRule(code_points=[1], forwarding_class_name="be1")])
	...
	print "################################################################################################################################"
    	...
    	api_request = CosRewrite(rewrite_name="cos1", rewrite_type=classifier_rewrite_type_DSCP_IPV6,)# rule=[RewriteRule(code_points=[1], forwarding_class_name="be1")])
    	result = cos.RewriteDelete(api_request, api_timeout)
	print 'Invoking cos.RewriteDelete \nreturn = ', result 
