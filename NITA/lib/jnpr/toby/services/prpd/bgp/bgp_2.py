...
def bgp_init(dev, channel_id, **kwargs):
    ...
    """
    if 'default_address_format' in kwargs and api_user != 'test':
        addrformat = kwargs.get('default_address_format')
    else:
        addrformat = None

    if 'default_table_format' in kwargs and api_user != 'test':
        tableformat = kwargs.get('default_table_format')
    else:
        tableformat = None
    initreq = prpd.InitializeRequest()
    if addrformat is not None and api_user != 'test':
        initreq.reply_address_format = addrformat
    if tableformat is not None and api_user != 'test':
        initreq.reply_table_format = tableformat

    initres = prpd.process_api(dev, channel_id, "Initialize", [initreq, "jnx_Bgp"])
...
def bgp_route_add(dev, channel_id, **kwargs):
    ...
    for i in dest_prefix_iterator:
        ...
        addres = prpd.process_api(dev, channel_id, "RouteAdd", [updreq, "jnx_Bgp"])
...
def bgp_route_modify(dev, channel_id, **kwargs):
    ...
    for i in dest_prefix_iterator:
        ...
        modres = prpd.process_api(dev, channel_id, "RouteModify", [updreq, "jnx_Bgp"])
...
def bgp_route_update(dev, channel_id, **kwargs):
    ...
    for i in dest_prefix_iterator:
        ...
        updres = prpd.process_api(dev, channel_id, "RouteUpdate", [updreq, "jnx_Bgp"])
...
def bgp_route_remove(dev, channel_id, **kwargs):
    ...
    for i in dest_prefix_iterator:
        ...
        remres = prpd.process_api(dev, channel_id, "RouteDelete", [remreq, "jnx_Bgp"])
...
def bgp_route_get(dev, channel_id, **kwargs):
    ...
    for i in dest_prefix_iterator:
        ...
        getres = prpd.process_api(dev, channel_id, "RouteGet", [getreq, "jnx_Bgp"])
...
#Building the Nexthop for the RIB Route Params
    ...
    for destprefix in dest_prefix:
        ...
        if routedata is not None and api_user != 'test':
            routeparams.addr_family_data.CopyFrom(routedatagen(routedata, table, api_user))
...
def  prefix_gen(dest_table, prefix, api_user, prefix_type=None):
    ...
    elif dest_table == 'inet.3':
        destpfx = prpd.RoutePrefix(labeled_inet=prpd.IpAddress(addr_string=prefix["dest_prefix"]))
    elif dest_table == 'inet6.3':
        destpfx = prpd.RoutePrefix(labeled_inet6=prpd.IpAddress(addr_string=prefix["dest_prefix"]))    
    #elif dest_table == 'inet.3':
        #destpfx = prpd.RoutePrefix(inet=prpd.IpAddress(addr_string=prefix["dest_prefix"]))    
    #elif dest_table == 'inet6.3':
...
def bgp_generate_remove_req(**kwargs):
    ...
    remreq = prpd.RouteDeleteRequest()
    if remlist is not None:
        remreq.routes.extend(remlist)
...
def bgp_generate_get_req(**kwargs):
    ...
    bgproutegetreq = prpd.RouteGetRequest()
    if getentry is not None and api_user != 'test':
        bgproutegetreq.route.CopyFrom(getentry[0])
    if orlonger is not None and api_user != 'test':
    ...
    """if addrformat is not None and api_user != 'test':
        bgproutegetreq.default_address_format = addrformat
    if tableformat is not None and api_user != 'test':
...
def bgp_monitor_register(dev, channel_id, **kwargs):
    ...
    monitorreq = prpd.RouteSubscribeRequest()
    """if addrformat is not None and api_user != 'test':
    ...
    monitor_stream = prpd.process_api(dev, channel_id, "RouteSubscribe", [monitorreq, "jnx_Bgp"])

    return monitor_stream

def bgp_monitor_unregister(dev, channel_id, **kwargs):
    """
    BGP monitor unregister to stop monitoring
    """

    monitorreq = prpd.RouteUnsubscribeRequest()
    monitor_unreg_reply = prpd.process_api(dev, channel_id, "RouteUnsubscribe", [monitorreq, "jnx_Bgp"])
