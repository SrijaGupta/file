...
def rib_route_modify(dev, channel_id, **kwargs):
    ...
    for i in dest_prefix_iterator:
        ...
        result.append(prpd.process_api(dev, channel_id, "RouteModify", [updreq, "Rib"], kwargs.get("timeout", None)))
...
def route_attribute(attribute):
    ...
    attribute_obj = prpd.RouteAttributeUint32()
...
def rib_route_remove(dev, channel_id, **kwargs):
    ...
    ** ONLY THE ROUTES ADDED BY A UNIQUE client_id CAN BE REMOVED
    ...
    for i in dest_prefix_iterator:
        ...
        remreq = prpd.RouteRemoveRequest(keys=rtkeys)
        result.append(prpd.process_api(dev, channel_id, "RouteRemove", [remreq, "Rib"], kwargs.get("timeout", None)))
...
def weight_translate(weight_value):
    """
    Module to translate enum weight
    """
    if weight_value == "WEIGHT_PRIMARY":
        weight_int = prpd.WEIGHT_PRIMARY
    elif weight_value == "WEIGHT_BACKUP":
        weight_int = prpd.WEIGHT_BACKUP
    elif weight_value == "WEIGHT_NONE":
        weight_int = prpd.WEIGHT_NONE
    else:
