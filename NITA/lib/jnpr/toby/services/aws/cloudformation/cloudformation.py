...
class Cloudformation(Aws):
    ...
    def create_cfn_stack(self, stackname,tempurl, parameters):
        """
            ...
            :param parameters:
                ...
                From robot: 
                ${PARAMETERS}=  Create Dictionary       vMXName=${EC2_NAME}          AvailZone=${ZONE}              SSHKey=${SSHKEY}
                ${stacks} =         Create Cloudformation Stack         ${aws_handle}  ${STACK}     ${TEMPURL}          ${PARAMETERS}
        ...
        try:
            response_create_stack = self.client_cfn.create_stack(StackName = stackname,
                TemplateURL = tempurl,
    ...
    def update_cfn_stack(self, stackname,tempurl, parameters):
        ...
        try:
            ...
            raise Exception(sys.exc_info()[0])
        try:
            response = self.client_cfn.update_stack(StackName = stackname,
                TemplateURL=tempurl,
                Parameters=param_list,
                NotificationARNs=[
                ],
    ...
    def stop_terminate_instance(self,id):
        """
    ...
    def del_cfn_stack(self,stackname):
        ...
        """
        response_delete_stack = self.client_cfn.delete_stack(
            StackName=stackname
        )
    ...
    def ver_ec2_state(self,id,state,waittime=900):
        """
            Verifies the state of ec2 instance against the given state running/stopped/terminated/shutting-down.
        ...
        while iteration <= waittime:
            ...
            for instance in instances:
                ...
                if id == instance.id :
                    ...
                    logging.info(id,instance.id,instance.tags)
                    status=True
                    break
            if status:
                break
            ...
            logging.info(status)
        return self.status
    def ver_ec2_status(self,id,state,waittime=900):
        """
        ...
        while iteration <= waittime:
            ...
            for status in self.resource_ec2.meta.client.describe_instance_status()['InstanceStatuses']:
        ...
        while iteration < waittime:
            try:
                ...
                if state=='DELETE_COMPLETE':
                    ...
                    raise Exception(sys.exc_info()[0])
        return False
    def ssh_pub(self,hostname,myuser,mySSHK,cmd):
        """
            Function to ssh a device after creation, which is not present in params.yaml,  using sshkey file
    ...
    def delete_ssh_key_pair(self, keypair):
        ...
        try:
            ...
            raise Exception(sys.exc_info()[0])
        return True
    def verify_vpn_status(self,vgwid,state,waittime=3600):
        """
        ...
        while iteration <= waittime:
            try:
                ...
                status1 = vpns['VpnConnections'][0]['VgwTelemetry'][0]['Status']
                logging.info("State of the vpn0:")
                logging.info(status1)
                status2 = vpns['VpnConnections'][1]['VgwTelemetry'][0]['Status']
                logging.info("State of the vpn1:")
                logging.info(status2)
                if status1.lower() == state.lower() and status2.lower() == state.lower():
                    return True
                time.sleep(5)
                iteration= iteration + 1
            except:
...
def connect_to_aws(*args, **kwargs):
    ...
    """
    return Cloudformation(*args, **kwargs)
...
def delete_eni (aws_handle, *args, **kwargs):
    """
        ...
        :return True/False:
            ...
            Else False
    """
    return aws_handle.delete_eni(*args, **kwargs)
