...
set security ike gateway gw-</xsl:text>
      ...
      <xsl:text>
set security ike gateway gw-</xsl:text>
      <xsl:value-of select="$id" /><xsl:text> external-interface ge-0/0/0.0
set security ike gateway gw-</xsl:text>
      <xsl:value-of select="$id" /><xsl:text> address </xsl:text>
      <xsl:value-of select="$gateway" />

#set static routing options through gateway 10.2.1.1 for both the tunnel addresses

<xsl:text>
set routing-options static route </xsl:text>
      <xsl:value-of select="$gateway" /><xsl:text> next-hop 10.2.1.1 </xsl:text>
