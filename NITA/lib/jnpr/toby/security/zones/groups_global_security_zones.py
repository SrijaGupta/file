...
def configure_groups_global_zone(zone=None, device=None, interface=None, host_inbound_traffic=False,
                                 system_service=None, protocol=None, address=None,
                                 address_book=False, address_set=None, ip_prefix=None,
                                 dns_name=None, range_address_low=None, range_address_high=None,
                                 wildcard_address=None, commit=False):
    """
    Configure groups global security zone

    :Example:

    python: configure_groups_global_zone(zone="trust", device=router_handle, interface=ge-0/0/1)

    robot:

    configure groups global zone    device=${dh}    zone=trust
    configure groups global zone    device=${dh}    zone=trust    interface=ge-0/0/1
    configure groups global zone    device=${dh}    zone=trust    interface=ge-0/0/1
        host_inbound_traffic=${True}    system_service=all
    configure groups global zone    device=${dh}    zone=trust    interface=ge-0/0/1
        host_inbound_traffic=${True}    protocol=all
    configure groups global zone    device=${dh}    zone=trust    host_inbound_traffic=${True}
        system_service=${service}
    configure groups global zone    device=${dh}    zone=trust    host_inbound_traffic=${True}
        protocol=${protocol}
    configure groups global zone    device=${dh}    zone=trust    address_book=${True}
    ...
    if zone is None:
        device.log(level="ERROR", msg="'zone' is mandatory parameter"
                                      " for configuring groups global zones")
        raise Exception("'zone' is mandatory parameter for configuring groups global zones")

    if device is None:
        raise Exception("'device' is mandatory parameter device handle")

    if zone is not None and device is not None:
        cmd = 'set groups global security zones security-zone ' + zone
