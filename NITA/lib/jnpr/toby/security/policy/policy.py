...
def configure_policy(device=None, from_zone=None, to_zone=None, policy_name=None,
                     source_address=None, destination_address=None,
                     application=None, action=None, default_policy_action=None,
                     logical_systems=None, global_policy=False, scheduler_name=None,
                     default_policy=False, commit=False):
    """
    Configuring policy configuration on the box

    :Example:
    python: configure_policy(device=router_handle, from_zone=trust, to_zone=untrust,
                                               policy_name=p1, commit=True)
    robot:
    configure policy    device=router_handle    from_zone=trust
                                   to_zone=untrust    policy_name=p1    commit=${TRUE}

    Configure Policy    device=${dh}    default_policy=${True}    default_policy_action=deny
    Configure Policy    device=${dh}   logical_systems=LSYS1    global_policy=${True}
        policy_name=p1    source_address=source    destination_address=destination
            application=any    action=permit
    Configure Policy    device=${dh}    from_zone=trust    to_zone=untrust   policy_name=p1
        scheduler_name=daily_scheduler
    Configure Policy    device=${dh}   global_policy=${True}
        policy_name=p1    source_address=${source}    destination_address=${destination}
            application=any    action=permit
    Configure Policy    device=${dh}    from_zone=trust    to_zone=untrust   policy_name=p1
    ...
    cmd = 'set security policies '

    if logical_systems is not None:
        cmd = 'set logical-systems ' + logical_systems + ' security policies '
