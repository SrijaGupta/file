...
class Openssl:
    ...
    def __init__(self, linux_handle, **kwargs):
        ...
        if self.sign:
            ...
            self.cert_file = self.ca_cert + '.csr'
        ...
        self.host_ip = self.linux_handle.shell(command='hostname -i').response() #get liunx ip
    ...
    def gen_openssl_ca_cert(self, **kwargs):
        ...
        for dir1 in (self.ca_path, self.private, self.cert_dir, self.newcerts,
                    self.crl_dir, self.private):
        ...
        self._invoke('cp -f ' + self.ca_path + '/' + 'openssl.cnf.TRUE' +  ' '  + self.ca_openssl_cnf)
        ...
        if self.sign:
            ...
            update_index_file(self.linux_handle, self.cert_dir + '/' + self.cert_file, self.ca_path + '/index.txt')
        return True
    ...
    def gen_cert_req(self, handle,**kwargs):
        ...
        """
        gen_on = kwargs.get('gen_on', 'router')
        if gen_on == 'router':
    ...
    def sign_cert(self,**kwargs):
        ...
        if hasattr(self, 'local_cert'):
            ...
            cmd = 'cp ' + self.ca_path + '/openssl.cnf.FALSE' + ' ' + self.ca_path + '/openssl.cnf'
            self._invoke(cmd)
            ...
            cmd = 'cp ' + self.ca_path + '/openssl.cnf.TRUE' + ' ' + self.ca_path + '/openssl.cnf'
            self._invoke(cmd)
        ...
        with open("openssl.cnf", "a") as myfile:
            subject_alt = 'subjectAltName=DNS:' + self.domain_name + ',' + 'email:' + self.email
            if self.ip != '0.0.0.0':
        ...
        if hasattr(self, 'local_cert'):
            sign_cli = 'openssl x509 -req -days ' + self.days + ' -in ' + self.ca_path + '/' + self.req_file +\
                       ' -CA ' + self.cert_dir + '/' + self.cert_file + ' -CAkey ' + self.key + \
            ...
            sign_cli = 'openssl x509 -req -days ' + self.days + ' -in ' + self.ca_path + '/' + \
            self.req_file + ' -CA ' + cert + ' -CAkey ' + private_key + \
            ' -CAcreateserial -out ' + self.cert_dir + '/' + sign_cert + ' -extfile ' +  self.ca_path + '/openssl.cnf ' + self.hash_algo
        ...
        if hasattr(self, 'local_cert'):
            update_index_file(self.linux_handle, self.cert_dir + '/' + sign_cert,
                               self.ca_path + '/index.txt')
        else:
            update_index_file(self.linux_handle, self.cert_dir + '/' + self.cert_file,
    ...
    def load_cert(self, **kwargs):
        ...
        cert = kwargs.get('cert_name', self.cert_file)
...
def gen_crl(linux_handle, **kwargs):
    ...
    :return: True on success, False on Failure
    EXAMPLE::
        gen_crl(linux_ahndle, cert_name='test_root_ca')
    """
    cert_name = ''
    if 'cert_name' in kwargs:
...
def start_ocsp_responder(linux_handle,cert_name):
    """
        Starts oscp servcie for teh cert
    :param linux_handle:
        **REQUIRED** linux handle object
    :param cert_name:
        **REQUIRED** cert_name
...
def update_index_file(linux_handle, cert_file, index_file):
    ...
    myfile.close()
    if not linux_handle.upload(local_file='index.txt', remote_file=index_file,
                               protocol='scp',user='root', password='Embe1mpls'):
...
def create_openssl_object(linux_handle, **kwargs):
    """
        Wrapper function calls openssl init class
    """
